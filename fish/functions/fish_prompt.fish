...
function __bobthefish_prompt_user -S -d 'Display current user and hostname'
    ...
    if set -q display_hostname
        if set -q display_user
            ...
            # set_color -b red blue
            echo -ns '@' (prompt_hostname)
        else
            # Ian: we want to set the background color based on the first character of the hostname
            # This gives a unique and consistent color per host without needing to hardcode things.
            # What I want is `0x006 rol ord(hostname[0])` but we don't have rol, so we emulate using
            # mul and mod
            ...
            if [ (prompt_hostname) = 'catbin' ]
                ...
                __bobthefish_start_segment 060
            else
                __bobthefish_start_segment 000
            end
            echo -ns (prompt_hostname)
        end
    end
